...
E = [f_solution(1), f_solution(2), f_solution(3); 
...
figure();
ImageMatrix = imread('nan0.jpg');
%ImageMatrix = imread('389_13.0081.jpg');
imshow(ImageMatrix);
hold on;
%hold("on");
...
plot3(debugdisp(:,1), debugdisp(:,2),debugdisp(:,3));
 ...
 figure;
 x= debugdisp(2,1:3)-debugdisp(1,1:3);
 r=vrrotvec2mat(vrrotvec([0,0,-1],x/(x*x')));
 result3=debugdisp(:,1:3);
 ax = [1,-2.8,0];
 r2 = vrrotvec2mat(vrrotvec([1,0,0],ax/(ax*x')));
 result3x = result3*r;
 result3x = result3x*r2;

 plot3(result3x(1:paircount,1), result3x(1:paircount,2),result3x(1:paircount,3));
